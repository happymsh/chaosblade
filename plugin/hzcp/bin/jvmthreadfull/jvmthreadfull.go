...
func main() {
	flag.BoolVar(&startFlag, "start", false, "start hzcpjvmthreadfull")
	flag.BoolVar(&stopFlag, "stop", false, "stop hzcpjvmthreadfull")
	...
	if keyword == "" {
		logrus.Error("hzcp-jvmthreadfull: target Java process match error: keyword is empty")
	...
	if pids != nil && len(pids) == 1 {
		logrus.Info("hzcp-jvmthreadfull: ", pids)
		pid = pids[0]
	} else if len(pids) > 0 {
...
var logFile = util.GetNohupOutput(util.Bin, "hzcpjvmthreadfull.log")
var channel = cl.NewLocalChannel()

const hzcpJvmThreadJarName string = "thread-1.0-jar-with-dependencies.jar"

// start burn io
func start() {
	hzcpJar := path.Join(util.GetProgramPath(), "hzcp", hzcpJvmThreadJarName)
	ctx := context.Background()
...
func stop() {
	ctx := context.Background()
	pids, _ := cl.GetPidsByProcessName(hzcpJvmThreadJarName, ctx)
