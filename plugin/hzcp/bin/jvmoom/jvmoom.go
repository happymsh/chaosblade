...
func main() {
	flag.BoolVar(&startFlag, "start", false, "start hzcpjvmoom")
	flag.BoolVar(&stopFlag, "stop", false, "stop hzcpjvmoom")
	...
	if keyword == "" {
		logrus.Error("hzcp-jvmoom: target Java process match error: keyword is empty")
	...
	if pids != nil && len(pids) == 1 {
		logrus.Info("hzcp-jvmoom pid:", pids)
		pid = pids[0]
	} else if len(pids) > 0 {
...
var logFile = util.GetNohupOutput(util.Bin, "hzcpjvmoom.log")
var channel = cl.NewLocalChannel()

const hzcpJvmOomJarName string = "heap-1.0-jar-with-dependencies.jar"

// start burn io
func start() {
	hzcpJar := path.Join(util.GetProgramPath(), "hzcp", hzcpJvmOomJarName)
	ctx := context.Background()
...
func stop() {
	ctx := context.Background()
	pids, _ := cl.GetPidsByProcessName(hzcpJvmOomJarName, ctx)
